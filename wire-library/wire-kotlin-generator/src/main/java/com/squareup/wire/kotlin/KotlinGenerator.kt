...
class KotlinGenerator private constructor(
  ...
  private val ProtoType.isListValue
    get() = this == ProtoType.STRUCT_LIST
  private val ProtoType.isValue
    get() = this == ProtoType.STRUCT_VALUE
  private val ProtoType.isNullValue
  ...
  private val Field.typeNameForMessageField: TypeName
    get() {
      ...
      return when (encodeMode!!) {
        ...
        EncodeMode.OMIT_IDENTITY -> {
          when {
            type.isStruct -> type.typeName
            type.isListValue -> type.typeName
            type.isValue -> type.typeName.copy(nullable = true)
            type.isNullValue -> type.typeName.copy(nullable = true)
    ...
    get() {
      return when (encodeMode!!) {
        ...
        EncodeMode.OMIT_IDENTITY -> {
          ...
          if (protoType.isStruct) return CodeBlock.of("emptyMap<String, Any?>()")
          if (protoType.isListValue) return CodeBlock.of("emptyList<Any?>()")
          if (protoType.isValue) return CodeBlock.of("null")
          if (protoType.isNullValue) return CodeBlock.of("null")
    ...
    get() {
      ...
      return when (encodeMode!!) {
        ...
        EncodeMode.OMIT_IDENTITY -> {
          when {
            type.isStruct -> false
            type.isListValue -> false
            type.isValue -> true
            type.isNullValue -> true
            isOneOf -> true
            type.isMessage -> true
            else -> false
          }
